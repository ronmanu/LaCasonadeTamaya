            ...
            {navItems.map((item) => (
              <button
                ...
                className={`text-sm font-medium transition-colors duration-200 ${
                  currentPage === item.value ? 'text-wood-600 border-b-2 border-wood-600' : 'text-stone-600 hover:text-wood-600'
                }`}
              >
                {item.label}
              </button>
          ...
          <div className="px-4 pt-8 pb-3 space-y-6 flex flex-col items-center">
            {navItems.map((item) => (
              <button
                key={item.label}
                onClick={() => handleNav(item.value)}
                className={`block px-3 py-2 text-xl font-serif ${
                  currentPage === item.value ? 'text-wood-600' : 'text-stone-800'
                }`}
              >
                {item.label}
              </button>

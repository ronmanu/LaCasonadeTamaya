...
export const StaffArea: React.FC<StaffAreaProps> = ({ onLogout }) => {
    ...
    const addCharge = async (e: React.FormEvent) => {
        ...
        const { error } = await supabase.from('room_charges').insert([{
            room, concept, amount, quantity: q, unit_price: up
        }]);
    ...
    const clearCharges = async () => {
        if (confirm('¿VACIAR TODO EL HISTORIAL?')) {
            const { error } = await supabase.from('room_charges').delete().neq('id', '00000000-0000-0000-0000-000000000000');
            ...
            <div className="min-h-screen pt-20 bg-stone-100 flex items-center justify-center px-4">
                <div className="max-w-md w-full bg-white rounded-2xl shadow-2xl p-8 border border-stone-200">
                    ...
                    <form onSubmit={handleLogin} className="space-y-6">
                        <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-xl outline-none focus:border-stone-900" placeholder="Contraseña" required />
        ...
        <div className="min-h-screen pt-20 bg-stone-50">
            <header className="bg-white border-b border-stone-200 py-6 px-4 mb-8">
                <div className="max-w-7xl mx-auto flex justify-between items-center">
                    <h1 className="text-xl font-serif font-bold text-stone-900">Panel Interno Tamaya</h1>
                    ...
                    <section className="bg-white rounded-3xl shadow-sm border border-stone-200 overflow-hidden flex flex-col h-[650px]">
                        <div className="p-6 border-b border-stone-100 bg-stone-50/50">
                            <div className="flex justify-between items-center mb-4">
                                ...
                                <button onClick={clearCharges} className="text-stone-300 hover:text-red-500 transition-colors"><Trash2 size={18} /></button>
                            </div>
                            <div className="flex gap-2 items-center">
                                <span className="text-xs font-bold text-stone-400 uppercase">Filtrar Hab:</span>
                                <select value={selectedRoomFilter} onChange={e => { setSelectedRoomFilter(e.target.value); setNewCharge({ ...newCharge, room: e.target.value }); }} className={`px-3 py-1.5 border rounded-lg text-sm font-bold ${selectedRoomFilter ? 'bg-stone-900 text-white' : 'bg-white'}`}>
                        ...
                        <form onSubmit={addCharge} className="p-4 border-b border-stone-100 bg-stone-50/20 flex flex-wrap gap-2">
                            <select value={newCharge.room} onChange={e => setNewCharge({ ...newCharge, room: e.target.value })} className="w-20 px-2 py-2 border border-stone-200 rounded-lg text-sm" required>
                                <option value="">Hab.</option>
                                {ROOMS.map(r => <option key={r.id} value={r.name.split(' ')[1]}>{r.name.split(' ')[1]}</option>)}
                            </select>
                            <input type="text" placeholder="Producto" value={newCharge.concept} onChange={e => setNewCharge({ ...newCharge, concept: e.target.value })} className="flex-1 px-3 py-2 border border-stone-200 rounded-lg text-sm" required />
                            <input type="number" placeholder="Cant." value={newCharge.quantity} onChange={e => setNewCharge({ ...newCharge, quantity: e.target.value })} className="w-16 px-2 py-2 border border-stone-200 rounded-lg text-sm" required />
                            <div className="flex gap-1 items-center">
                                <input type="number" step="0.01" placeholder="Precio" value={newCharge.unitPrice} onChange={e => setNewCharge({ ...newCharge, unitPrice: e.target.value })} className="w-20 px-2 py-2 border border-stone-200 rounded-lg text-sm" required />
                                <span className="text-stone-300">€</span>
                                <button type="submit" className="bg-stone-900 text-white p-2 rounded-lg"><Plus size={18} /></button>
                            </div>
                        </form>
                        ...
                        <div className="flex-grow overflow-y-auto p-6 space-y-3 bg-stone-50/30">
                            {filteredCharges.length === 0 ? (
                                ...
                                filteredCharges.map((c) => (
                                    <div key={c.id} className="group flex items-center justify-between p-3 bg-white rounded-xl border border-stone-100 shadow-sm hover:border-stone-300">
                                        <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <button onClick={() => c.id && deleteCharge(c.id)} className="text-stone-300 hover:text-red-500 p-1 group-hover:opacity-100 opacity-0 transition-opacity"><X size={16} /></button>
                                        </div>
                                    </div>
                                ))
                            )}
                        ...
                        <div className={`p-4 text-white text-center text-xs font-bold uppercase tracking-widest ${selectedRoomFilter ? 'bg-wood-700' : 'bg-stone-900'}`}>
                            {selectedRoomFilter ? `Total Hab. ${selectedRoomFilter}: ` : 'Total General: '}
